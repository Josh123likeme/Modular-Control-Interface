#include "../mci.h"

#include <avr/pgmspace.h>

const uint8_t rotary_encoder_map[256] PROGMEM = {
  };

uint8_t get_single_knob_rotation_value(module_t* module) {

    if (module->module_id != SINGLE_KNOB_MODULE_ID) return 255;
    
    if (automatic_output_update_enabled) update_module_outputs();

    uint8_t decoded_rotation = (pgm_read_byte(rotary_encoder_map + module->module_output) + 2) % 128;

    return decoded_rotation;

}

float get_single_knob_rotation_ratio(module_t* module) {

    if (module->module_id != SINGLE_KNOB_MODULE_ID) return -1.0f;
    
    if (automatic_output_update_enabled) update_module_outputs();

    uint8_t decoded_rotation = (pgm_read_byte(rotary_encoder_map + module->module_output) + 2) % 128;

    return ((float) (decoded_rotation) / SINGLE_KNOB_MAX_VALUE);

}