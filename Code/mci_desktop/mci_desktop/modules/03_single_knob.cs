using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace mci_desktop.modules
{
    public class _03_single_knob : Module
    {

        public static readonly byte MODULE_ID = 0x03;

        private readonly int[] rotary_encoder_map = new int[] {
          };

        public static readonly int SINGLE_KNOB_INCREMENTS = 117;
        public static readonly int SINGLE_KNOB_MAX_VALUE = (SINGLE_KNOB_INCREMENTS - 1);

        internal _03_single_knob(byte[] modulePointer) : base(modulePointer)
        {
        }

        public int getRotationValue()
        {

            int decoded_rotation = (rotary_encoder_map[moduleOutput] + 2) % 128;

            return decoded_rotation;
        }

        public double getRotationRatio()
        {

            int decoded_rotation = (rotary_encoder_map[moduleOutput] + 2) % 128;

            return ((float)(decoded_rotation) / SINGLE_KNOB_MAX_VALUE);

        }

        public override byte getModuleID()
        {
            return 0x03;
        }
    }
}
